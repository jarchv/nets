...
class VQGAN(nn.Module):
    ...
    def load_epoch(self, epoch, log_num):
        if epoch <= 0:
            return
        ...
        print('\nLoading "vqgan_nodisc-{:d}"...'.format(epoch), end='')
        file_model = 'vqgan_nodisc-{:d}.pth'.format(epoch)

        load_path  = os.path.join(checkpoints_path, file_model)
        checkpoint = torch.load(load_path)

        self.load_state_dict(checkpoint['state_dict_net'])
